...
#
#         # 使用 torchvision 中的 ResNet-50 作为 trunk
#         self.trunk = resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)
#
#         # 修改 trunk 的最后层以适应上采样模块
#         self.trunk = nn.Sequential(*list(self.trunk.children())[:-2])  # 截取到最后的卷积层
#
#         # 上采样模块
#         self.up1 = Up(2048 + 1024, 512)  # 调整输入和输出通道数
#         if downsample == 8:
...
#         endpoints = {}
#
#         # 使用截取的 `Sequential` 中的索引来访问子模块
#         x = self.trunk[0](x)  # conv1
#         x = self.trunk[1](x)  # bn1
#         x = self.trunk[2](x)  # relu
#         x = self.trunk[3](x)  # maxpool
#         endpoints['reduction_1'] = x
#
#         # 逐层访问 `layer1` 到 `layer4` 的输出
#         x = self.trunk[4](x)  # layer1
#         endpoints['reduction_2'] = x
#
#         x = self.trunk[5](x)  # layer2
#         endpoints['reduction_3'] = x
#
#         x = self.trunk[6](x)  # layer3
#         endpoints['reduction_4'] = x
#
#         x = self.trunk[7](x)  # layer4
#         endpoints['reduction_5'] = x
#
#         # 进行上采样，将 higher-level 特征图与 lower-level 特征图连接
...
#             else:
#                 return None, None, None, x_img, new_x
...
class BevEncodeMSFusion(nn.Module):
    ...
    def __init__(self, fusion_args):  # inC: 64  outC: not 1 for object detection
        ...
        elif fusion_args['core_method'] == "attn_ms":
            self.fuse_module = AttFusion(args['att']['feat_dim'])
        elif fusion_args['core_method'] == "disconet_ms":
            self.fuse_module = DiscoFusion(args['disconet']['feat_dim'])
        elif fusion_args['core_method'] == "v2vnet_ms":
            self.fuse_module = V2VNetFusion(args['v2vnet'])
        elif fusion_args['core_method'] == 'v2xvit_ms':
            self.fuse_module = V2XViTFusion(args)
        elif fusion_args['core_method'] == 'cobevt_ms':
            self.fuse_module = CoBEVT(args['cobevt'])
        elif fusion_args['core_method'] == 'where2comm_ms':
            self.fuse_module = Where2commFusion(256)
        else:
...
class V2XViTFusion(nn.Module):
    def __init__(self, args):
        ...
        self.fusion_net = V2XTransformer(args['v2xvit']['transformer'])
