...
def project_world_objects_dairv2x(object_list,
    ...
    for object_content in object_list: 
        ...
        # corners_world = boxes_to_corners_3d()
        ...
        # bbx_lidar = np.expand_dims(bbx_lidar[:, :3], 0) # [1, 8, 3]
        # bbx_lidar = corner_to_center(bbx_lidar, order=order)
        dimensions = object_content['3d_dimensions']
        location = object_content['3d_location']
        rotation = object_content['rotation']
        bbx_lidar = np.expand_dims(np.array([location['x'], location['y'], location['z'], dimensions['h'], dimensions['w'], dimensions['l'], rotation]),0)
