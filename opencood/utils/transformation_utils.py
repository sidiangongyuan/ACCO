...
def generate_K_from_json_file(json_file):
    cam_K_values = json_file['cam_K']
    cam_D_values = json_file['cam_D']
    K = np.array(cam_K_values).reshape(3, 3)
    cam_D = np.array(cam_D_values)

    return K,cam_D


def camera_to_world_coords(x1,x2):
    T_camera_to_world = np.dot(x2, x1)

    return T_camera_to_world


def json_file_transformation_matrix_load(json_file):
    matrix = np.empty([4, 4])
    rotation = json_file["rotation"]
    translation = json_file["translation"]
