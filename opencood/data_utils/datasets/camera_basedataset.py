...
class CameraBaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.preload = True \
            if ('preload' in params['train_params'] and params['train_params']['preload']['enabled']) \
            else False
    ...
    def retrieve_base_data(self, idx):
        ...
        cav_id_list = [cav_id_list[0]] + cav_id_list_  # 保持第一个Cav ID不变
    ...
    def retrieve_per_process(self, proc_id, idx_list, lock, preload_shared_dict):
        base_data_list = []
        for idx in idx_list:
            base_data_list.append(self.retrieve_base_data(idx))  # 确保这个函数不依赖于外部状态
        with lock:
            preload_shared_dict[proc_id] = base_data_list
    ...
    def retrieve_all_base_data_mp(self):
        ...
        with Manager() as manager:
            lock = Lock()
            preload_shared_dict = manager.dict()  # 使用Manager创建共享字典
            ...
            for i in range(self.preload_worker_num):
                p = Process(target=self.retrieve_per_process, args=(i, idx_list_split[i], lock, preload_shared_dict))
            ...
            for i in range(self.preload_worker_num):
                self.all_base_data.extend(preload_shared_dict[i])

            preload_shared_dict.clear()  # Optionally clear the shared dictionary
